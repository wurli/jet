...
function M:get(from, to)
	local ret = {} ---@type table<number, Jet.Ui.Image.Placement>
	local marks = vim.api.nvim_buf_get_extmarks(self.buf, require("jet.core.ui.image.placement").ns, { from - 1, 0 }, { to, -1 }, {
		overlap = true,
		hl_name = false,
	})
...
function M:update()
	local conceal = require("jet.core.ui.image.config").doc.conceal
	conceal = type(conceal) ~= "function" and function()
		return conceal
	end or conceal
	require("jet.core.ui.image.doc").find_visible(self.buf, function(imgs)
		...
		for _, i in ipairs(imgs) do
			...
			if not img then
				...
				img = require("jet.core.ui.image.placement").new(
					self.buf,
					i.src,
					Snacks.config.merge({}, require("jet.core.ui.image.config").doc, {
						pos = i.pos,
						range = i.range,
						inline = true,
