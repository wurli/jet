...
---@field cmd (fun(step: Jet.Ui.Image.step):(Jet.Ui.Image.Proc|Jet.Ui.Image.Proc[]))|Jet.Ui.Image.Proc|Jet.Ui.Image.Proc[]
---@field ft? string
---@field file? fun(convert: Jet.Ui.Image.Convert, meta: Jet.Ui.Image.meta): string
...
local commands = {
	...
	tex = {
		ft = "pdf",
		file = function(convert, ctx)
			ctx.pdf = require("jet.core.ui.image.config").cache .. "/" .. vim.fs.basename(ctx.src):gsub("%.tex$", ".pdf")
	...
	mmd = {
		cmd = {
			cmd = "mmdc",
			args = require("jet.core.ui.image.config").convert.mermaid,
		},
		file = function(convert, ctx)
	...
	convert = {
		ft = "png",
		cmd = function(step)
			local formats = vim.deepcopy(require("jet.core.ui.image.config").convert.magick or {})
			...
			vim.list_extend(
				args,
				{ "-write", "{file}", "-identify", "-format", "%m %[fx:w]x%[fx:h] %xx%y", "{file}.info" }
			)
			return {
...
function Convert.new(opts)
	vim.fn.mkdir(require("jet.core.ui.image.config").cache, "p")
	...
	self.steps = {}
	self.tpl_data = {
		cache = require("jet.core.ui.image.config").cache,
		bg = vim.o.background,
		scale = tostring(require("jet.core.ui.image.terminal").size().scale or 1),
	}
	self:resolve()
	return self
end
...
function Convert:tmpfile(ft)
	return require("jet.core.ui.image.config").cache .. "/" .. self.prefix .. "." .. ft
end
...
function Convert:on_done()
	...
	self._done = true
	if self._err and require("jet.core.ui.image.config").convert.notify then
...
function Convert:step()
	...
	step.proc = Spawn.new({
		run = false,
		debug = require("jet.core.ui.image.config").debug.convert,
