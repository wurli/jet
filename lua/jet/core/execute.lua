...
M.get_curr_line = function()
	return { code = { vim.api.nvim_get_current_line() } }
end
...
M.get_visual = function()
	local mode = vim.fn.mode()
	if vim.tbl_contains({ "v", "V", "" }, mode) then
		return { code = vim.fn.getregion(vim.fn.getpos("v"), vim.fn.getpos("."), { type = mode }) }
	end
	return M.get_curr_line()
end
...
---```
---
---@param callback fun(code: Jet.Execute.Code)
---@return fun(): "g@"
...
M._handle_curr_motion = function(mode)
	...
	_G.JET_OP_PENDING_CALLBACK({ code = code })
