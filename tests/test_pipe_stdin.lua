...
T["chansend over plain pipe stdin delivers every line"] = function()
  ...
  local job_id = vim.fn.jobstart({ jet_bin, "start", kernel_json }, {
    ...
    on_stdout = function(_, data, _) for _, l in ipairs(data) do table.insert(output, l) end end,
    on_stderr = function(_, data, _) for _, l in ipairs(data) do table.insert(output, l) end end,
  })
