...
local kernel_id, info = jet.connect(spec)
...
local poll = jet.execute_code(kernel_id, "print(1+1)", {})
local saw = ""
...
assert(saw:find("2"), string.format("expected '2' in stream output, got: %q", saw))
